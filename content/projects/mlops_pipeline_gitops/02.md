+++
date = '2025-10-06T17:10:21+09:00'
draft = false
...
| 2 | 🔁 ArgoCD 동기화 (ApplicationSet) | — |
| 3 | 🎯 Airflow 학습→등록→READY 흐름 | 3.1 Variables 설정
3.2 MLflow 연동 결과(FAIL)
3.3 FAIL Slack 알림
3.4 SUCCESS(dev/prod)
3.5 FastAPI 모델 로드(dev/prod)
3.6 등록/핫스왑 Slack(dev/prod)
3.7 Self-Heal 테스트 |
| 4 | 🗂️ MLflow 등록/별칭/아티팩트 | 4.1 Variables의 별칭/롤백 파라미터
4.2 Registry 별칭 바인딩(dev/prod)
4.3 S3 아티팩트(dev/prod) |
| 5 | ⚡ FastAPI 핫스왑 | 5.1 `/variant/{alias}/reload`(dev/prod)
5.2 핫스왑 Slack 알림(dev/prod) |
| 6 | 🔄 수동 롤백(운영 복구 레일) | 6.1 롤백 변수 설정
6.2 롤백 전 상태
6.3 롤백 DAG 결과(dev/prod)
6.4 롤백 후 상태 |
| 7 | 📑 로그 스토리지(S3+NFS) | 7.1 Airflow 로그 S3
7.2 FastAPI 로그 NFS |
| 8 | 🛰️ ArgoCD↔️Slack 관제 루프 | — |
| 9 | 🔐 보안 자동화(크리덴셜/시크릿)
9.1 AWS Credentials Secret Rotation
9.2 SealedSecret Controller & Re-seal | 9.1.a  Before 상태
9.1.b 이전 키 중복 감지 실패 사례
9.1.c 회전 및 ArgoCD 반영 완료
9.1.d 리소스 버전 비교 증명
9.1.e ArgoCD 최종 상태 검증
9.2.a  컨트롤러키 재발급 및 fingerprint 검증 
9.2.b  ArgoCD 반영 확인 |
| 10 | 🚦 트래픽 실험 (A/B·Canary·Blue-Green) | 스크립트 `/ops/ab_test.sh`, `/ops/capture_test.sh`
10.1 A/B 테스트
10.2 Canary 테스트(예: 30%)
10.3 Blue-Green 전환 |
| 11 | 🔗 One-Commit Flow 전체 체인 | 11.1 준비
11.2 브랜치/PR 트리거
11.3 CI/CD 통합 검증
11.4 PR 머지 & 자동 Sync
11.5 Variables 자동 반영
11.6 학습→등록→핫스왑→Slack
11.7 Canary 검증
11.8 Blue-Green 검증
11.9 로그 계층 검증
11.10 AWS 회전 검증
11.11 SealedSecrets 키 회전
11.12 롤백 변수 설정
11.13 롤백 실행 & Slack |
