...
## 에필로그 — “GitOps 기반 Triton 서빙이 ‘배포→검증→관측→알림’ 루프로 고정되다”
...
| 단계 | 핵심 목표 | 주요 개선점 |
| --- | --- | --- |
| 1 | Triton “서빙 플랫폼 뼈대” 구축 | GitOps로 dev/prod 분리 배포 + explicit load<br>NFS model-repo + /metrics 관측 |
| 2 | 자동 배포 파이프라인 운영화 | MLflow Registry(단일 소스)<br>→ materialize<br>→ load/ready/infer 검증<br>→ commit_current, 실패 시 minimal rollback |
| 3 | 알람 신뢰성/분리 표준화 | Alertmanager null default<br>namespace 정규식 허용만 Slack 전송<br>Triton mean latency 기반 알럿 도입 |
...
- **GitOps 배포로 Triton dev/prod가 안정적으로 떠 있고**
- **Airflow가 MLflow Registry에서 모델을 내려 받아 model-repo에 배치**
- **Triton explicit load로 “원할 때만” 모델을 로딩**
- **ready + smoke infer 검증을 통과한 경우에만 current.json에 운영 상태를 확정(commit)**
- **메트릭은 Prometheus/Grafana에서 관측되고**
- **알람은 Alertmanager가 최종 통제(null default)하며 Slack은 분리 채널로만 수신**
- 실패 시 **운영 기준(current.json) 복구 + 실패 버전 격리 + best-effort unload/load**로 최소 롤백
...
- GitHub 코드: [[GitOps] mlops-platform-triton](https://github.com/keonhoban/mlops-infra-labs/tree/main/mlops-platform-triton)
