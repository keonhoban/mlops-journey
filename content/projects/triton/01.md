...
| 2 | [🔗 MLflow → Triton 자동 배포 파이프라인 구축 (Airflow 기반· 검증 체인· 최소 롤백)](https://keonhoban.github.io/mlops-journey/posts/triton/02/) |
...
| 단계 | 핵심 목표 | 주요 개선점 |
| --- | --- | --- |
| 1 | Triton “서빙 플랫폼 뼈대” 구축 | GitOps로 dev/prod 분리 배포 + explicit load 
+ NFS model-repo + /metrics 관측 |
| 2 | 자동 배포 파이프라인 운영화 | MLflow Registry(단일 소스) 
→ materialize 
→ load/ready/infer 검증 
→ commit_current, 실패 시 minimal rollback |
| 3 | 알람 신뢰성/분리 표준화 | Alertmanager null default 
+ namespace 정규식 허용만 Slack 전송 
+ Triton mean latency 기반 알럿 도입 |
