...
| 단계 | 핵심 목표 | 주요 개선점 |
| --- | --- | --- |
| 1 | Triton 서빙 기반 구축 | GitOps dev/prod 분리 · explicit load · NFS model-repo · /metrics |
| 2 | 배포 파이프라인 운영화 | MLflow 단일 소스 → 검증 체인(load/ready/infer) → commit/rollback |
| 3 | 알람 신뢰성 표준화 | Alertmanager null default · namespace 기반 Slack 라우팅 · mean latency 알럿 |
