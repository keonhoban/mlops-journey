...
| 2 | 🔗 MLflow → Triton 자동 배포 파이프라인 구축 (Airflow 기반 · 검증 체인 · 최소 롤백) |
| 3 | 🔗 Alerting 운영 표준 매뉴얼 (Dev/Prod 철벽 분리 + Triton Serving Alerts) |
...
> 
> **검증 체인(load/ready/infer) + 최소 롤백(current.json 기준 복구) + 알람 라우팅(dev/prod 철벽 분리)** 으로 안정화했습니다.
> 
