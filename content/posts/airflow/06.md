...
- **KubernetesExecutor vs CeleryExecutor**: ML 파이프라인에서 Task별 환경 격리와 동적 리소스 할당이 핵심 요구사항이므로 KubernetesExecutor가 적합하다. Celery는 Broker(Redis/RabbitMQ) 운영 부담이 추가되고, Worker가 동일 환경을 공유하여 의존성 충돌 위험이 있다.
- **pod_template_file 명시적 정의 vs 기본값 사용**: 운영 환경에서는 ServiceAccount, 리소스 제한, 로그 PVC를 명시적으로 정의하여 예측 가능한 동작을 보장해야 한다. 기본값은 리소스 제한이 없어 Node 리소스를 과도하게 사용할 수 있다.
- **Role(Namespace) vs ClusterRole**: Task Pod가 같은 Namespace에서 실행되면 최소 권한 원칙에 따라 Role이 충분하다. ClusterRole은 불필요하게 넓은 권한을 부여한다.

---

## 면접 포인트

- **Q: KubernetesExecutor의 Cold Start 문제를 어떻게 완화하나?**
  → `imagePullPolicy: IfNotPresent`로 이미지 캐싱, 경량 베이스 이미지 사용, Node에 사전 이미지 배포. 10-30초 지연이 허용되지 않는 Task는 CeleryExecutor를 고려할 수 있다.

- **Q: Task Pod 로그가 사라지는 문제의 해결 방법은?**
  → NFS PVC 마운트 또는 Remote Logging(S3/GCS). 이 프로젝트에서는 초기에 NFS PVC를 사용하고, 규모가 커지면 Remote Logging으로 전환한다.

- **Q: CeleryExecutor 대비 KubernetesExecutor의 단점은?**
  → Cold Start 지연(Pod 생성 10-30초), 클러스터 리소스 부족 시 Pending 발생, RBAC 설정 필요. 대량의 짧은 Task가 빈번하면 CeleryExecutor가 유리할 수 있다.
