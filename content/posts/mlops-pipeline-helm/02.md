...
- **envFrom vs volumeMount 분리 이유**: MLflow/FastAPI는 단일 Pod에서 환경변수로 주입하면 충분하지만, Airflow는 KubernetesExecutor로 Task마다 새 Pod를 생성합니다. 이때 `envFrom`은 Task Pod에 전파되지 않을 수 있어 `.aws/credentials` 파일 마운트가 안정적입니다.
- **Secret + ConfigMap 분리 vs 통합 URI**: MLflow는 DB 접속 정보를 개별 환경변수로 조합해야 하므로 Secret(인증) + ConfigMap(호스트 정보)으로 분리합니다. Airflow는 `metadataSecretName`으로 단일 URI를 참조하는 구조라 통합 방식이 적합합니다.
- **SealedSecrets 언급 이유**: base64 인코딩은 암호화가 아닙니다. Git에 Secret YAML을 올리면 누구나 디코딩할 수 있으므로, 프로덕션에서는 SealedSecrets/SOPS/External Secrets Operator로 암호화 후 커밋해야 합니다.
- **Secret 주입 후 반드시 확인**: `kubectl create secret` 후 `kubectl describe secret` 또는 Pod 내 `env` 명령어로 실제 주입 여부를 확인합니다. 민감한 키가 담긴 파일은 `.gitignore`에 포함하고 작업 완료 후 삭제합니다.

---

## 면접 포인트

- **Q: Kubernetes Secret과 ConfigMap의 차이는?**
  → Secret은 base64 인코딩된 민감한 데이터(비밀번호, 키)를 저장하고, ConfigMap은 평문 설정값(호스트, 포트)을 저장합니다. Secret은 etcd에서 암호화 옵션을 지원하며, 노출 범위를 제한할 수 있습니다.

- **Q: Airflow에서 env 방식 대신 volumeMount를 사용하는 이유는?**
  → KubernetesExecutor는 Task마다 새 Pod를 생성하는데, `envFrom`으로 주입한 환경변수가 Task Pod에 전파되지 않을 수 있습니다. `.aws/credentials` 파일을 마운트하면 boto3가 기본 경로에서 자동으로 인식합니다.

- **Q: base64 인코딩이 보안인가요?**
  → 아닙니다. base64는 인코딩일 뿐 암호화가 아닙니다. `echo <값> | base64 -d`로 누구나 디코딩할 수 있으므로, Git에 올릴 때는 반드시 SealedSecrets 등으로 암호화해야 합니다.
