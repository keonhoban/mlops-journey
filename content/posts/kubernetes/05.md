+++
date = '2025-06-05T20:06:43+09:00'
draft = false
title = '[Kubernetes - 5단계: Prometheus + Grafana 모니터링]'
categories = 'Kubernetes'
+++

## 이 글에서 다루는 것

> Prometheus로 Kubernetes 클러스터 메트릭을 수집하고, Grafana 대시보드로 시각화하는 모니터링 기초를 다룹니다.

## 선수지식

- [Kubernetes 4단계: Helm](/mlops-journey/posts/kubernetes/04/) — Helm Chart 설치와 values.yaml 커스터마이징

---

## 이 단계에서 해결하려는 문제

Kubernetes 클러스터에서 Pod, Node의 CPU/메모리 사용량을 실시간으로 파악하지 못하면, 장애 대응과 리소스 최적화가 불가능합니다. Prometheus가 메트릭을 수집하고, Grafana가 이를 대시보드로 시각화합니다. 이 구조는 MLOps에서 모델 서빙 지연, 학습 리소스 모니터링의 기반이 됩니다.

실습 코드: [GitHub (Monitoring)](https://github.com/keonhoban/mlops-infra-labs/tree/main/k8s-basic/05_Prometheus_and_Grafana)

---

## 🧭 전체 흐름 요약

```
① Helm 저장소 추가
② Prometheus 설치 (메트릭 수집)
③ Grafana 설치 (대시보드 시각화)
④ Web UI 접속 → Prometheus 연결 → 대시보드 불러오기
```

---

## 📂 디렉토리 구성

```
k8s-monitoring/
├── values-prometheus.yaml   # Prometheus 커스터마이징 설정
├── values-grafana.yaml      # Grafana 설정 + 비밀번호 지정
├── README.md
```

---

## ⚙️ [1단계] Helm 저장소 추가

```bash
helm repo add prometheus-community https://prometheus-community.github.io/helm-charts
helm repo add grafana https://grafana.github.io/helm-charts
helm repo update
```

---

## 📦 [2단계] Prometheus 설치

### 기본 설치

```bash
helm install prometheus prometheus-community/prometheus
```

```bash
kubectl port-forward svc/prometheus-server 9090:80
```

접속: `http://localhost:9090`

---

### NodePort 설정 (선택)

```yaml
# values-prometheus.yaml
server:
  service:
    type: NodePort
    nodePort: 30091
```

```bash
helm install prometheus prometheus-community/prometheus -f values-prometheus.yaml
```

접속: `http://<minikube-ip>:30091`

---

## 📈 [3단계] Grafana 설치

### 기본 설치

```bash
helm install grafana grafana/grafana
kubectl port-forward svc/grafana 3000:80
```

접속: `http://localhost:3000`

- ID: `admin`
- PW: secret에서 추출

### Grafana 접속 비밀번호 확인 (secret 추출)

```bash
kubectl get secret grafana -o jsonpath="{.data.admin-password}" | base64 -d
```

또는 `values-grafana.yaml`에서 직접 지정:

### NodePort 설치 + 비밀번호 설정

```yaml
# values-grafana.yaml
service:
  type: NodePort
  nodePort: 30092
adminPassword: "admin1234"
```

```bash
helm install grafana grafana/grafana -f values-grafana.yaml
```

접속: `http://<minikube-ip>:30092` (ID: `admin` / PW: `admin1234`)

---

## 🧪 [4단계] Grafana에서 Prometheus 연결

1. 좌측 톱니바퀴 > **Data Sources**
2. **Prometheus** 선택 → URL: `http://prometheus-server`

---

## 📊 [5단계] 대시보드 가져오기

1. 좌측 + 버튼 > Import
2. Dashboard ID:
    - `315` (Kubernetes cluster monitoring)
    - `6417` (Node Exporter Full)

---

## ✅ 정리 요약

| 구성 요소 | 역할 |
| --- | --- |
| **Prometheus** | 쿠버네티스 자원 모니터링 메트릭 수집 |
| **Grafana** | 실시간 대시보드 시각화 도구 |
| **Helm** | 패키지 관리 (설치/업그레이드/삭제 자동화) |
| **NodePort/port-forward** | 웹 UI 접속 방식 선택 가능 |

---

## 설계 판단 (Why This Way?)

- **Prometheus + Grafana vs 상용 모니터링(Datadog 등)**: 오픈소스 조합이 비용 없이 커스터마이징 가능합니다. 프로덕션에서는 kube-prometheus-stack(Prometheus Operator)으로 더 체계적으로 구성합니다.
- **port-forward vs NodePort**: 개발 환경에서는 port-forward가 간편하고, 팀 공유가 필요하면 NodePort를 사용합니다. 프로덕션에서는 Ingress를 통해 노출합니다.
- **커뮤니티 대시보드 활용**: ID `315`, `6417`은 수천 명이 사용하는 검증된 대시보드입니다. 처음부터 만들지 않고 커뮤니티 리소스를 활용하는 것이 효율적입니다.
- **Prometheus Operator와의 차이**: 이 실습은 standalone Prometheus입니다. 프로덕션에서는 ServiceMonitor CRD를 사용하는 Operator 방식이 표준입니다.

---

## 면접 포인트

- **Q: Prometheus의 Pull 방식이란?**
  → Prometheus가 주기적으로 타겟의 `/metrics` 엔드포인트를 scrape합니다. Push 방식(Pushgateway)도 있지만, Pull이 기본입니다.

- **Q: kube-state-metrics와 node-exporter의 차이는?**
  → kube-state-metrics는 K8s 오브젝트 상태(Deployment replicas, Pod status 등), node-exporter는 노드 하드웨어 메트릭(CPU, 메모리, 디스크)을 수집합니다.

- **Q: Grafana 대시보드를 GitOps로 관리하는 방법은?**
  → 대시보드 JSON을 ConfigMap으로 저장하고, Grafana sidecar가 자동 로드하도록 구성합니다.

---

## 다음에 읽을 글

→ [Airflow 1단계: 로컬 환경에서 기본 DAG 실행](/mlops-journey/posts/airflow/01/) — Docker 기반 Airflow 환경 구성과 첫 DAG 실행
