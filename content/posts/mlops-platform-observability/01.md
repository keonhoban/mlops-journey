...
- **dev/prod 네임스페이스 분리 vs 단일 Prometheus + 라벨 분리**: 네임스페이스를 분리하면 ArgoCD Application, PVC, Secret이 모두 독립되어 한쪽을 완전히 날려도 다른 쪽에 영향이 없다. 단일 Prometheus에 라벨만 나누면 운영이 간결하지만, dev 실험이 prod 모니터링 성능에 영향을 줄 수 있다.
- **초기 NFS -> 이후 local-path 전환 전략**: 처음부터 local-path로 시작하면 구조는 최적이지만, NFS 하나로 통일해두면 초기 구축 속도가 빠르고 스토리지 의존성이 단순하다. TSDB I/O 병목이 실제로 체감된 뒤에 Prometheus만 local-path로 분리하는 것이 근거 있는 전환이 된다.
- **ArgoCD Auto Sync + SelfHeal**: 수동 Sync는 안전하지만, Observability 스택은 항상 떠 있어야 하므로 SelfHeal로 drift를 자동 복구하는 것이 운영 부담을 줄인다.

---

## 면접 포인트

- **Q: kube-prometheus-stack을 왜 dev/prod로 분리 배포했나?**
  → Prometheus selector, PVC, Secret이 모두 독립되어 한 환경의 장애가 다른 환경에 전파되지 않고, ArgoCD Application 단위로 독립 관리가 가능하다.

- **Q: Prometheus 스토리지로 NFS를 먼저 쓴 이유는?**
  → 초기에는 스토리지 계층을 단순화해서 구축 속도를 높이고, TSDB I/O 병목이 실제로 발생한 뒤에 local-path로 전환하는 것이 근거 있는 설계 판단이 된다.

- **Q: ArgoCD SelfHeal을 켠 이유와 위험성은?**
  → Observability 스택은 항상 가동 상태여야 하므로 drift 자동 복구가 필요하다. 다만 의도적인 수동 변경도 되돌려지므로, 모든 변경은 반드시 Git을 통해야 한다.
