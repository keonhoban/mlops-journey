...
```mermaid
flowchart TD
    R[S3 Raw CSV] --> E[extract_raw_data]
    E --> V[validate_data]
    V --> F[build_features]
    F --> S[store_features]
    S --> SUM[summarize_run]

    subgraph Airflow_DAG_v2
      E
      V
      F
      S
    ...
    F -->|XCom: feature_csv, schema, metadata| S
    S -->|feature.csv<br>schema.json<br>metadata.json| FB[S3 Feature Prefix<br>daily_user_events/v_타임스탬프/]
