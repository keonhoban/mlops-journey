...
- **v1을 제거하지 않고 v2를 병행한 이유**: 기존 DAG를 유지하면 v1 → v2 전환 중에도 운영이 중단되지 않는다. 두 버전을 나란히 실행하여 출력을 비교 검증한 뒤 v1을 비활성화하는 점진적 마이그레이션이 가능하다.
- **schema.json과 metadata.json을 분리한 이유**: schema는 컬럼/타입 정보로 변경 빈도가 낮고, metadata는 실행마다 달라지는 품질/계보 정보다. 분리하면 schema 변경 감지와 실행별 품질 추적을 독립적으로 처리할 수 있다.
- **버전 ID를 실행 시간 기반(v_YYYYMMDDTHHMMSS)으로 한 이유**: 순차 번호(v1, v2)보다 시간 기반 ID가 S3 콘솔에서 정렬/필터링에 유리하고, 실행 시점을 ID만으로 즉시 파악할 수 있다. 단, 동일 초에 두 번 실행되면 충돌할 수 있어 운영 스케줄 간격을 고려해야 한다.

---

## 면접 포인트

**Q. 데이터 파이프라인에서 버전 관리를 왜 S3 디렉터리 구조로 했나요?**
A. 별도 DB 없이 S3 prefix만으로 버전 비교, 롤백, 재현이 가능하다. schema.json과 metadata.json을 함께 두면 해당 버전의 컬럼 구조와 품질 이력까지 파일 시스템만으로 추적할 수 있다.

**Q. Feature lineage를 어떻게 추적하나요?**
A. metadata.json에 source_raw_path, rows_raw, null_rate, valid 등을 기록한다. 특정 Feature 버전이 어떤 Raw 데이터에서 왔고 품질이 어땠는지를 파일 하나로 즉시 확인할 수 있다.

**Q. KST 타임존을 명시적으로 관리하는 이유는?**
A. Airflow execution_date는 UTC 기반이므로 운영자가 보는 시간과 불일치가 생긴다. KST로 통일하면 로그/S3 구조/대시보드의 시간축이 일치하여 디버깅 시 혼란이 줄어든다.
