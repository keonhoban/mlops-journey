...
노드 수가 제한된 환경에서 물리적 클러스터 분리 대신 release 라벨 + metricRelabelings로 논리적 메트릭 격리를 구현했다. Prometheus TSDB는 고 IOPS가 필수이므로 local-path를, Grafana/Loki 등 I/O가 적은 워크로드는 NFS를 사용하여 스토리지를 워크로드 특성별로 분리했다.
