...
소규모 데이터에서는 XCom이 가장 단순한 Task 간 통신 수단이므로 S3 중간 저장의 추가 복잡도를 피했고, validate_data를 별도 Task로 분리하여 검증 실패 시 이후 파이프라인을 조기 차단한다. 숫자형 컬럼 후보를 명시적으로 하드코딩하여 스키마 자동 추론의 예측 불가능한 타입 변환을 방지했다.
