+++
date = '2025-10-18T17:10:21+09:00'
draft = false
title = '[MLOps 플랫폼 Observability & Data Pipeline - 2단계 : Alertmanager Slack & 트러블슈팅]'
categories = ['Observability', 'GitOps(ArgoCD)', 'Kubernetes', 'Monitoring Stack', 'Slack', 'NFS', 'Security', 'MLOps Pipeline', 'Trouble Shoot', 'AlertManager']
+++

## Alertmanager Slack & 트러블슈팅

---

## 🧠 시나리오 설명

> “모델이 잘 학습되고 잘 배포되는 것보다
> 
> 
> **문제가 생겼을 때 즉시 감지되는 것**이 더 중요할 때가 많습니다.”
> 
> “그래서 이번 단계에서는 dev/prod 각각에서 알람이
> 
> **정확한 Slack 채널로, 깨짐 없이 흐르는 관측 파이프라인**을 먼저 완성했습니다.”
> 
> “이 기반이 갖춰져야 이후 FastAPI 지연, 핫스왑 실패, DAG 에러 같은
> 
> 운영형 MLOps 이벤트를 실시간으로 감지할 수 있기 때문입니다.”
> 

---

### 🎯 핵심 요약

- **목표**
    - `Prometheus → Alertmanager → Slack` 경로를
        
        dev/prod 완전히 분리된 상태로,
        
        “깨지면 바로 원인 찾을 수 있는 수준”까지 정리
        
- **핵심 포인트**
    - Alertmanager 설정은 **SealedSecret**로 관리
    - `alertmanager.alertmanagerSpec.configSecret` 로 CR에 연결
    - dev/prod 라우팅 기준은 `namespace` 라벨 정규식
    - `null` receiver로 **Watchdog·불필요 알람 소음 차단**
    - `or` 템플릿 함수만 사용해서 `"function default not defined"` 에러 제거
- **결과물**
    - 새 클러스터에서도 GitOps만으로
        
        **Slack까지 알람 흐름을 100% 재현**할 수 있는 상태
        

---

## 1️⃣ 전체 구조 개요

![mermaid-observability-02.png](/mlops-journey/images/mermaid-observability-02.png)

### 🧩 구성 요소 역할

| 구성 요소 | 설명 |
| --- | --- |
| `PrometheusRule` | 알람 조건 정의 (Smoke 테스트, FastAPI 지연, CrashLoop 등) |
| `Prometheus` | Rule 평가 → firing 상태로 Alertmanager로 전달 |
| `Alertmanager` | 라벨 기준으로 Slack 또는 `null` 등으로 라우팅 |
| `Slack` | dev/prod 채널로 분리된 알림 수신 |

---

## 2️⃣ 사전 준비 체크리스트

### ✅ 인프라 전제

- [ ]  A1에서 구성한 `monitoring-dev / monitoring-prod` 네임스페이스와 kube-prometheus-stack 동작 중
- [ ]  `ArgoCD` 설치 및 `apps/monitoring-{dev,prod}.yaml` 로 GitOps 운영 중
- [ ]  `nginx-ingress` 및 도메인(예: `alert-dev.local`, `prometheus-dev.local`) 접근 가능

### ✅ Alertmanager / SealedSecret / Slack 전제

- [ ]  `sealed-secrets` 컨트롤러 설치 (`kube-system` 네임스페이스)
- [ ]  dev/prod용 Slack Webhook URL 각각 준비
- [ ]  Git 리포지토리 내에 `envs/dev/sealed-secrets/monitoring/`, `envs/prod/sealed-secrets/monitoring/` 디렉터리 존재

> 이 편에서는 앞단계에서 구축한 모니터링 스택 위에
> 
> 
> **Alertmanager–Slack 경로와 SealedSecret 관리**를 얹는 단계만 다룹니다.
> 

---

## 3️⃣ Alertmanager 설정 설계 (dev/prod 분리 기준)

### 3-1. 라우팅 기본 구조

핵심은 **namespace 라벨**을 기준으로 dev/prod를 명확히 나누는 것입니다.

예시(핵심 구조만):

```yaml
global:
  resolve_timeout: 5m

route:
  receiver: "null"               # 기본은 버림
  group_by: ["alertname", "namespace"]
  group_wait: 30s
  group_interval: 5m
  repeat_interval: 12h
  routes:
    - receiver: "slack-dev"      # dev용
      matchers:
        - 'namespace=~".*-dev|monitoring-dev|kube-system"'
    - receiver: "null"           # 기타는 버림

receivers:
  - name: "slack-dev"
    slack_configs:
      - api_url: "<SLACK_WEBHOOK_URL_DEV>"
        send_resolved: true
        title: "🚨 [{{ .Status | toUpper }}][DEV] {{ .CommonLabels.alertname }}"
        text: >
          *Namespace:* {{ or .CommonLabels.namespace "N/A" }}
          *Severity:* {{ or .CommonLabels.severity "none" }}
          *Summary:* {{ or .CommonAnnotations.summary "No summary" }}
          *Description:* {{ or .CommonAnnotations.description "No description" }}
          *Start:* {{ (index .Alerts 0).StartsAt }}
          <https://prometheus-dev.local/alerts|View in Prometheus>

  - name: "null"

```

> prod는 slack-prod, namespace=~".*-prod|monitoring-prod|kube-system" 패턴으로 동일 구조를 사용합니다.
> 

### 3-2. 왜 `or` 함수인가?

- Helm 템플릿에서 쓰는 `default`는 Alertmanager 템플릿에서는 **존재하지 않는 함수**
- Alertmanager는 **Go 템플릿의 내장 함수만 사용** 가능
- 그래서 다음과 같이 작성해야 오류 없이 동작합니다:

```yaml
{{ or .CommonLabels.namespace "N/A" }}

```

그렇지 않으면 Alertmanager 로그에:

> template: : function "default" not defined
> 

이 찍히고, 해당 설정은 **무시**됩니다.

---

## 4️⃣ SealedSecret 기반 Alertmanager Config 관리

### 4-1. 평문 Secret → SealedSecret 변환

dev 예시 기준입니다.

1. 우선 `/tmp/alertmanager-dev.yaml`에 위에서 설계한 내용을 저장합니다.
2. 그걸 Secret으로 만들되, **바로 적용하지 않고** YAML만 출력합니다.

```bash
kubectl -n monitoring-dev create secret generic alertmanager-config-dev \
  --from-file=alertmanager.yaml=/tmp/alertmanager-dev.yaml \
  --dry-run=client -o yaml > /tmp/alertmanager-config-dev.secret.yaml

```

1. 그 다음 SealedSecret으로 봉인:

```bash
kubeseal \
  --controller-namespace kube-system \
  --controller-name sealed-secrets \
  --format yaml < /tmp/alertmanager-config-dev.secret.yaml \
  > envs/dev/sealed-secrets/monitoring/alertmanager-config-dev.yaml

```

- prod도 동일하게 `alertmanager-config-prod.yaml`로 생성
- 중요한 체크 포인트:
    - `metadata.namespace: monitoring-dev / monitoring-prod`
    - `spec.template.type: Opaque`
    - `spec.encryptedData.alertmanager.yaml` 존재

> 이 파일만 Git에 올려도 Webhook 주소는 복호화 불가능해서 안전합니다.
> 

---

## 5️⃣ kube-prometheus-stack에 ConfigSecret 연결

앞단계에서 만든 `apps/monitoring-*.yaml`에 이미 이 필드가 있습니다:

```yaml
alertmanager:
  alertmanagerSpec:
    configSecret: alertmanager-config-dev   # dev

```

여기 오타가 나면 아래 문제가 생깁니다:

- `alertmanager-config-dev`가 아닌 다른 키 사용
- `alertmanager.configFromSecret` 같은 **존재하지 않는 값** 사용

그러면:

- Operator가 내부 기본 템플릿으로 `alertmanager-*-generated` Secret을 만들고
- 그 설정이 Pod 내 `/etc/alertmanager/config/alertmanager.yaml.gz` 에 마운트됩니다.

그래서 반드시 **이 필드**를 확인해야 합니다:

```bash
kubectl -n monitoring-dev get alertmanager monitoring-dev-kube-promet-alertmanager \
  -o jsonpath='{.spec.configSecret}'; echo
# 기대값: alertmanager-config-dev

kubectl -n monitoring-prod get alertmanager monitoring-prod-kube-prome-alertmanager \
  -o jsonpath='{.spec.configSecret}'; echo
# 기대값: alertmanager-config-prod

```

> 이게 다르면 아무리 SealedSecret을 잘 만들어도 런타임에는 반영되지 않습니다.
> 

---

## 6️⃣ Runtime 상태 확인 (필수 Runbook)

### 6-1. Pod 안에서 실제 적용된 설정 보기

```bash
POD=$(kubectl -n monitoring-dev get po -l app.kubernetes.io/name=alertmanager -o name | head -1)

kubectl -n monitoring-dev exec "$POD" -c alertmanager -- \
  sh -lc 'zcat /etc/alertmanager/config/alertmanager.yaml.gz | sed -n "1,200p"'

```

여기서 확인해야 할 것:

- `route.routes[].matchers`에 dev용 정규식 존재
- `receivers`에 `slack-dev` / `"null"` 등 원하는 이름 그대로 있는지
- Webhook URL은 마스킹되더라도 구조가 유지되는지

### 6-2. Alertmanager API로 현재 Config 확인

```bash
curl -sk https://alert-dev.local/api/v2/status \
| jq -r '.config.original' | yq '.route, .receivers'

```

- `receiver: slack-dev`
- dev용 `namespace` 정규식
- `null` receiver 존재 여부

---

## 7️⃣ Slack 연동 검증 시나리오

### 시나리오 표

| 단계 | 검증 포인트 | 확인 방법 |
| --- | --- | --- |
| 1 | ConfigSecret 연결 | Alertmanager CR의 `.spec.configSecret` |
| 2 | Alertmanager가 설정 제대로 읽는지 | `/api/v2/status` |
| 3 | Smoke Rule → Prometheus → Alertmanager | `/api/v1/alerts`, `/api/v2/alerts` |
| 4 | Slack으로 실제 알람 도착 | dev/prod 채널에서 메시지 확인 |
| 5 | 전송 실패율 | `alertmanager_notifications_failed_total` 메트릭 |
| 6 | 재시작 / ArgoCD Sync 후에도 유지되는지 | Rollout + Sync 후 다시 테스트 |

---

## 8️⃣ Smoke Alert (PrometheusRule로 전체 경로 테스트)

### 8-1. dev용 Smoke Rule

```bash
cat <<'EOF' | kubectl -n monitoring-dev apply -f -
apiVersion: monitoring.coreos.com/v1
kind: PrometheusRule
metadata:
  name: alertmanager-smoke
  labels:
    release: monitoring-dev
spec:
  groups:
    - name: smoke.rules
      rules:
        - alert: Smoke_Alert_To_Slack
          expr: vector(1)
          for: 1m
          labels:
            severity: warning
            namespace: monitoring-dev     # 🔴 라우팅 키
          annotations:
            summary: "Smoke test alert"
            description: "This is a test alert to verify Slack integration"
EOF

```

### 8-2. Prometheus → Alertmanager → Slack 확인

```bash
# 1) Prometheus 알람 상태
curl -sk https://prometheus-dev.local/api/v1/alerts \
| jq '.data.alerts[] | select(.labels.alertname=="Smoke_Alert_To_Slack") | {state:.state,labels:.labels}'

# 2) Alertmanager에서 receiver 확인
curl -sk https://alert-dev.local/api/v2/alerts \
| jq '.[] | select(.labels.alertname=="Smoke_Alert_To_Slack") | {status:.status,receiver:.receiver.name}'

```

- `state: "firing"`
- `receiver: "slack-dev"`

이 두 가지면 pipeline은 정상입니다.

Slack 채널에는 다음과 비슷한 메시지가 떠야 합니다:

> 🚨 [FIRING][DEV] Smoke_Alert_To_Slack
> 
> 
> Namespace: monitoring-dev
> 
> Severity: warning
> 
> …
> 

테스트 후 정리:

```bash
kubectl -n monitoring-dev delete prometheusrule alertmanager-smoke

```

---

## 9️⃣ Alertmanager 직접 POST 테스트 (빠른 연기 검증)

Prometheus를 거치지 않고 라우팅만 빨리 보고 싶을 때 쓰는 방법입니다.

### dev 채널 테스트

```bash
curl -sk -X POST https://alert-dev.local/api/v2/alerts \
  -H 'Content-Type: application/json' \
  -d '[
    {
      "labels": {
        "alertname": "SlackTestDev",
        "severity": "critical",
        "namespace": "airflow-dev"
      },
      "annotations": {
        "summary": "🔔 Slack 테스트 알람 (dev / direct)",
        "description": "Alertmanager 직접 POST 테스트"
      }
    }
  ]'

```

확인:

```bash
curl -sk "https://alert-dev.local/api/v2/alerts?filter=alertname%3DSlackTestDev" \
| jq '.[].labels'

```

> prod도 alert-prod.local, namespace: airflow-prod 로 동일하게 테스트 가능합니다.
> 

---

## 🔟 전송 성공률/실패율 모니터링

Slack Webhook이 막히거나 템플릿이 깨졌을 때 **숫자로 감지**하는 영역입니다.

```bash
for env in dev prod; do
  echo "=== $env ==="

  echo "✅ Slack 전송 성공률 (5m)"
  curl -skG "https://prometheus-${env}.local/api/v1/query" \
    --data-urlencode 'query=sum(rate(alertmanager_notifications_total{integration="slack"}[5m]))' \
  | jq '.data.result[]?.value'

  echo "❌ Slack 전송 실패율 (5m, 이유 포함)"
  curl -skG "https://prometheus-${env}.local/api/v1/query" \
    --data-urlencode 'query=sum by (reason)(rate(alertmanager_notifications_failed_total{integration="slack"}[5m]))' \
  | jq '.data.result[]?'
done

```

- 기대 상태
    - 성공률: 0보다 큰 값
    - 실패율: 0 또는 0.00X 수준

---

## 🧩 팁

### 자주 터지는 이슈 & 트러블슈팅 정리

| 증상 | 원인 | 해결책 |
| --- | --- | --- |
| `failed to initialize from secret: missing name in receiver` | `route`에서 `"null"` 리시버를 참조하는데, `receivers:` 블록에 `name: "null"` 없음 | `receivers`에 `name: "null"` 추가 |
| Alertmanager 로그: `function "default" not defined` | 템플릿 내부에서 Helm의 `default` 사용 | Go 템플릿 `or`로 교체 (`{{ or .CommonLabels.namespace "N/A" }}`) |
| `/api/v2/alerts`에서 `receiver: null` | 라우트 matchers 조건 불일치 | Smoke Rule에 `namespace` 라벨 추가, `matchers` 정규식 다시 점검 |
| dev에서 prod 네임스페이스 알람 보임 | dev/prod 라벨·selector 섞임 (`release: monitoring-dev` 오류 등) | kube-prometheus-stack values에서 `serviceMonitorSelector`, `kube-state-metrics.additionalLabels` 정정 |
| Watchdog 알람이 Slack에 계속 옴 | Watchdog 전용 분기 없음 | `alertname=Watchdog` 인 경우 `receiver: "null"` 로 라우트 분기 |
| 설정을 바꿨는데도 예전 템플릿이 계속 보임 | `alertmanager-*-generated` 시크릿이 계속 사용됨 | `spec.configSecret` 값을 올바르게 수정하고, generated Secret 삭제 후 Alertmanager 롤링 재시작 |

---

## 🔧 MLOps 실전 연결

- 이 Alertmanager–Slack 라인은 나중에:
    - **FastAPI p95 지연**, 에러율,
    - Airflow DAG 실패,
    - MLflow 모델 등록 실패,
    - 모델 핫스왑 실패,
- 이런 것들을 **실시간으로 잡아내는 관측 알람의 백본**이 됩니다.
- 특히 이 Alertmanager–Slack 라인을:
    - Airflow → MLflow → FastAPI **핫스왑 / 롤백 파이프라인**과 연결하면,
    - “학습/배포 자동화” 뿐 아니라
        
        **“알람 → 롤백/핫스왑 후 상태 확인”**까지 이어지는
        
        **완전한 MLOps 운영 루프**를 보여줄 수 있습니다.
        

---

## 🏁 정리

| 항목 | 상태 |
| --- | --- |
| Alertmanager Config 관리 방식 | dev/prod 각각 SealedSecret(`alertmanager-config-{dev,prod}`)로 Git에 저장 |
| Alertmanager CR 연동 | `.spec.configSecret` 이 dev/prod에서 각자 올바른 Secret을 가리킴 |
| Alertmanager 실제 설정 | Pod 내부 `/etc/alertmanager/config/alertmanager.yaml.gz` 및 
`/api/v2/status` 로 확인 가능 |
| Smoke Alert 경로 | Prometheus `/api/v1/alerts` + Alertmanager `/api/v2/alerts` 에서 
`state: firing`, `receiver=slack-<env>` 확인 |
| Slack 채널 분리 | dev/prod 채널 각각에서 알람 수신 확인 |
| 전송 실패율 모니터링 | `alertmanager_notifications_failed_total{integration="slack"}` ≒ 0 유지 |
| 재배포/Sync 후 일관성 | Alertmanager 롤링 재시작 및 ArgoCD Sync 이후에도 동일 설정 유지 |
