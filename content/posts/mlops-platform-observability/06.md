...
대시보드를 서비스/플랫폼/모델 3종으로 분리하여 장애 시 on-call 엔지니어가 핵심 지표에 즉시 접근하도록 했다. Alert 임계값은 보수적으로 시작하여 실 트래픽 기반으로 튜닝하고, Prometheus(무엇이 문제)와 Loki(왜 문제) Alert를 병행하여 원인 분석 시간을 단축한다.
