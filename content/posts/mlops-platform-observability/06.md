...
- alert: FastAPIHighErrorRate
  expr: (sum(rate(fastapi_request_count{status!~"2.."}[2m])) /
         sum(rate(fastapi_request_count[2m]))) > 0.05
  for: 1m
  labels:
    severity: critical
  annotations:
    summary: "FastAPI 에러율 증가"
    description: "5xx 또는 비정상 응답 비율이 5% 초과"
...
- alert: FastAPIP95LatencyHigh
  expr: histogram_quantile(0.95,
          sum(rate(fastapi_request_duration_seconds_bucket[2m]))
          by (le)) > 0.1
  for: 2m
  labels:
    severity: warning
...
앞단계에서 정리한 Alertmanager 구성 구조를 그대로 사용합니다.
