...
- **Feast 이전에 Feature Store-lite를 먼저 구축한 이유**: Feast는 Registry/Online Store/Feature Server 등 운영 부담이 큽니다. 도구를 먼저 도입하면 "스키마가 뭔지, 버전이 뭔지"도 정하지 않은 채 인프라만 복잡해집니다. 계약(스키마) + 버전화 + 재현성 메타를 먼저 고정하면, Feast 도입 시 그 위에 얹기만 하면 됩니다.

- **스키마를 ConfigMap으로 GitOps 배포하는 이유**: 스키마를 코드에 하드코딩하면 DAG 수정 없이 스키마 변경이 불가능합니다. ConfigMap으로 분리하면 ArgoCD sync만으로 dev/prod 동시 배포 + 롤백이 가능하고, Airflow는 읽기만 합니다. 대안인 Airflow Variable은 UI에서만 수정 가능해 Git 추적이 안 됩니다.

- **버전 디렉토리를 1단 고정한 이유**: 날짜/실행ID/태스크ID를 섞어 depth를 늘리면 "최신 버전이 무엇인지" 찾기가 어렵습니다. `<feature_set>/<version>/` 1단 구조는 ls 한 번으로 전체 버전 목록을 확인할 수 있고, latest 포인터 도입도 자연스럽습니다.

---

## 면접 포인트

**Q. Feature Store에서 "재현성"이란 구체적으로 무엇인가요?**
A. 특정 모델이 학습에 사용한 피처가 "어떤 스키마, 어떤 소스, 어떤 시점에 생성된 것인지"를 시스템적으로 증명할 수 있는 것입니다. schema_hash + metadata.json이 이 역할을 합니다.

**Q. DAG를 얇게 유지해야 하는 이유는?**
A. DAG에 비즈니스 로직이 들어가면 수정/테스트/재사용이 모두 DAG 파일에 묶입니다. 로직을 패키지로 분리하면 단위 테스트가 가능하고, DAG는 의존성 그래프만 유지합니다.
