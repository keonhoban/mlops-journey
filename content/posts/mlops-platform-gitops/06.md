...
- **PythonSensor mode=reschedule vs mode=poke**: `poke`는 워커 슬롯을 점유한 채 대기한다. `reschedule`은 슬롯을 반환하고 다음 poke 시점에 재스케줄링되어 리소스 효율이 좋다. 모델 READY 감시처럼 대기 시간이 긴 센서에 적합하다.
- **TriggerRule.ALL_SUCCESS로 reload 보호**: 등록 실패나 센서 실패 시에도 reload가 실행되면 잘못된 모델이 서빙될 수 있다. 성공 체인에서만 실행되도록 보장한다.
- **cert-manager self-signed vs Let's Encrypt**: 내부망 환경에서는 외부 CA 접근이 불가하므로 self-signed가 현실적이다. 인증서 갱신은 cert-manager가 자동 처리하므로 운영 부담이 없다.

---

## 면접 포인트

- **Q: Airflow PythonSensor의 mode="reschedule"과 "poke"의 차이는?**
  → `poke`: 워커 슬롯 점유 상태로 polling. `reschedule`: poke 사이에 슬롯 반환. 대기 시간이 긴 센서(모델 READY 감시)에서는 reschedule이 리소스 효율적이다.

- **Q: Ingress TLS termination의 의미는?**
  → TLS 복호화를 Ingress(nginx)에서 수행하고, 내부 백엔드 통신은 HTTP로 유지한다. 인증서 관리가 한 곳으로 집중되고, 백엔드 Pod는 TLS 설정 없이 동작할 수 있다.

- **Q: TriggerRule.ALL_SUCCESS를 사용하지 않으면 어떤 위험이 있나?**
  → 기본값 `ALL_DONE`은 upstream이 실패해도 실행된다. 모델 등록이 실패한 상태에서 `/reload`가 실행되면 잘못된 버전이 서빙될 수 있다.
