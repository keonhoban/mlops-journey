...
## 모델 롤백 자동화: 등록 실패 대비 복구 루프 완성

---

## 🧠 시나리오 설명

> “모델 배포는 성공했을 때보다, **실패했을 때의 회복 속도**가 운영 품질을 결정합니다.”
> 
> 
> “그래서 3단계에서는 등록이 실패하는 즉시,
> 
> 이전 안정 버전으로 돌아가 FastAPI에 자동 반영되는 **완전한 롤백 루프**를 구축했습니다.”
> 
> “이 구조가 있어야 장애 상황에서도 서비스가 **끊기지 않고 바로 복구**될 수 있습니다.”
> 
...
sequenceDiagram
    ...
    AF->>MF: 새 버전 등록 시도 (create_model_version → set alias)
    MF-->>AF: ❌ 실패
    AF->>AF: before_version = get_model_version_by_alias(model, alias)
    AF->>MF: rollback_model(model, before_version, alias)
    AF->>FA: POST /variant/{alias}/reload
    FA-->>AF: ✅ {"status":"success", ...}
    AF-->>SL: "Register 실패 → 이전 버전 복구(+reloaded)"
...
> 의도: 서비스 가용성 최우선. 등록 실패 분기에서 즉시 롤백 + FastAPI 반영 후, 예외는 재전파해 원인 기록.
> 
...
def register_model_task(ti, **_):
    ...
    try:
        ...
        try:
            if prev_version:
                rollback_model(model_name, prev_version, alias)   # alias 되돌리기
                trigger_reload(alias)                              # FastAPI 반영
        ...
        raise  # DAG 실패로 남겨 원인 추적(전역 실패 콜백 동작)
...
> 내부 설정 헬퍼를 사용해 URL·토큰을 불러오며, 응답은 status == "success" 를 기대합니다.
> 
> 
> (타임아웃 5초)
> 
...
- [ ]  등록 **직전** `before_version` 확보 (XCom 없이 함수 내부에서 즉시 변수화 권장 - 가장 최근값 비교)
...
> “배포의 품질은 복구 시간으로 증명된다.”
> 
> 
> 자동 롤백(실패 브랜치 내 `/reload` 포함)과 수동 롤백 DAG를 함께 갖추면,
> 
> 장애 상황에서도 **명확한 대상 버전**으로 **즉시 복구**하고,
> 
> 계약된 응답(`status="success"`)으로 **서비스 반영**까지 확실히 보장할 수 있습니다.
>
