...
AWS 키처럼 유출 시 즉각 비용이 발생하는 시크릿만 자동 회전하고, 영향 범위가 큰 JWT/Webhook은 수동 유지하여 운영 안전성을 확보했다. SealedSecret re-encrypt로 평문 노출 없이 키 갱신이 가능하고, 시크릿 교체는 운영자가 실행 시점을 결정하는 반자동 방식을 택했다.
