...
- **자동화 대상 선별 원칙 (자주+위험 vs 가끔+영향 큰 것)**: AWS 키는 유출 시 즉각적 비용 발생이므로 자동 회전. JWT/Slack Webhook은 교체 시 세션 무효화/관제 단절 위험이 있어 수동 유지한다.
- **SealedSecret re-encrypt vs 재생성**: `re-encrypt`는 평문을 건드리지 않고 최신 공개키로 재암호화만 하므로 안전하다. 재생성은 평문을 다시 준비해야 하므로 유출 위험이 증가한다.
- **반자동(스크립트 + 수동 트리거) vs 완전 자동(CronJob)**: 시크릿 교체는 영향 범위가 크므로 운영자가 실행 시점을 결정하는 것이 안전하다. 자동 CronJob은 예기치 않은 시점에 교체가 발생할 수 있다.

---

## 면접 포인트

- **Q: SealedSecret 컨트롤러 키 교체 시 기존 시크릿은 어떻게 되나?**
  → 컨트롤러 키가 바뀌면 기존 암호문을 복호화할 수 없다. `re-seal`(재암호화)을 수행해야 한다. re-seal은 클러스터에서 기존 암호를 복호화한 뒤 새 키로 재암호화하므로 평문이 외부에 노출되지 않는다.

- **Q: AWS Access Key 회전 절차에서 구 키를 즉시 삭제하지 않는 이유는?**
  → 새 키가 모든 서비스에 반영(ArgoCD sync + Pod restart)되기 전에 구 키를 삭제하면 일시적 접근 실패가 발생한다. 검증 후 비활성화, 이후 삭제하는 2단계 절차가 안전하다.

- **Q: SealedSecret의 장점과 한계는?**
  → 장점: Git에 안전하게 커밋 가능, GitOps 호환. 한계: 클러스터 의존(키 교체 시 re-seal 필요), 대규모 시크릿 관리에는 ESO/Vault가 더 적합하다.
