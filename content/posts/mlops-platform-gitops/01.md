...
- **MLflow Alias 기반 라우팅 vs URL 기반 버전 지정**: Alias(@A/@B)로 추상화하면 모델 버전이 바뀌어도 서빙 코드를 수정할 필요가 없다. URL에 버전을 넣으면 매번 배포가 필요하다.
- **단일 FastAPI 앱에서 3가지 전략 vs 전략별 서비스 분리**: 하나의 앱에서 환경변수(`ALIAS_SELECTION_MODE`)로 전략을 전환하면 배포 없이 실험 정책을 바꿀 수 있다. 서비스를 분리하면 격리는 좋지만 운영 복잡도가 증가한다.
- **3단계 헬스체크(ok/degraded/unhealthy) vs 단순 binary**: `degraded` 상태를 두면 A/B 중 하나만 실패해도 서비스를 유지하면서 알림을 보낼 수 있다. binary라면 부분 장애에서도 전체 서비스가 내려간다.

---

## 면접 포인트

- **Q: A/B 테스트와 Canary 배포의 차이는?**
  → A/B는 사용자 그룹별 고정 분배(실험 목적), Canary는 새 버전으로 점진적 트래픽 이동(안전 배포 목적). 이 프로젝트에서는 SHA256 해시 기반으로 두 전략을 하나의 라우팅 로직에서 처리한다.

- **Q: MLflow Alias를 모델 전환 기준으로 사용하는 이유는?**
  → 버전 번호가 아닌 역할(A/B)로 바인딩하면, 새 버전 등록 시 alias만 재할당하면 되어 서빙 코드 변경이 불필요하다.

- **Q: FastAPI 기동 시 A/B 모델을 동시 로딩하는 이유는?**
  → 하나만 실패해도 `degraded` 상태로 서비스를 유지할 수 있고, 전부 실패하면 `unhealthy`로 즉시 감지된다.
