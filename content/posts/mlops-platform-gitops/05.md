...
- **NFS PVC vs emptyDir vs stdout-only**: `emptyDir`은 Pod 재시작 시 로그가 소실된다. stdout-only는 `kubectl logs`로만 확인 가능하고 Pod 삭제 후 검색이 불가하다. NFS PVC는 Pod 라이프사이클과 독립적으로 로그를 보존하며, 여러 Pod에서 동시 쓰기(`ReadWriteMany`)가 가능하다.
- **Loguru vs 표준 logging 모듈**: Loguru는 rotation, retention, 구조화 포맷, 비동기 쓰기를 설정 한 줄로 처리할 수 있다. 표준 `logging` 모듈로 동일 기능을 구현하려면 `RotatingFileHandler` + `TimedRotatingFileHandler` + 포맷터 설정이 별도로 필요하다.
- **enqueue=True (비동기 쓰기)**: NFS I/O 지연이 FastAPI 요청 처리를 차단하는 것을 방지한다. 다만 Pod 비정상 종료 시 버퍼에 남은 로그가 유실될 수 있으므로, 크리티컬 이벤트는 Slack 알림으로 이중 보장한다.

---

## 면접 포인트

- **Q: Kubernetes에서 로그를 NFS에 저장하면 어떤 문제가 있나?**
  → NFS I/O 지연이 로그 쓰기를 차단할 수 있다. `enqueue=True`(비동기 쓰기)나 버퍼링으로 완화하고, 심각한 경우 Remote Logging(S3)으로 전환한다. NFS 서버 장애 시 Pod 기동 자체가 차단될 수 있으므로 마운트 타임아웃 설정도 필요하다.

- **Q: Pod가 삭제된 후 로그를 어떻게 확인하나?**
  → PVC에 로그가 남아 있으므로 다른 Pod에서 같은 PVC를 마운트하거나 NFS 서버에서 직접 확인할 수 있다. `persistentVolumeReclaimPolicy: Retain`으로 PVC 삭제 후에도 데이터가 보존된다.

- **Q: stdout 로깅과 파일 로깅을 동시에 사용하는 이유는?**
  → 개발 시에는 `kubectl logs`로 즉시 확인이 필요하고, 운영 시에는 파일로 영속화하여 사후 분석에 사용한다. `LOG_TO_STDOUT` 환경변수로 환경별로 제어한다.
