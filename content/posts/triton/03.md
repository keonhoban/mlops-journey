...
- **Default receiver를 "null"로 고정한 이유**: 기본을 Slack으로 두면 라벨 실수 하나로 prod 알럿이 dev 채널로 가거나, 무관한 알럿이 쏟아집니다. "기본은 차단, 허용된 것만 전송"이 안전한 출발점이며, 화이트리스트 방식이 블랙리스트 방식보다 운영 사고가 적습니다.

- **p95 대신 mean latency를 표준으로 잡은 이유**: Triton 기본 메트릭이 histogram이 아니라 sum/count 형태이므로 p95를 네이티브로 뽑을 수 없습니다. custom histogram을 만드는 복잡성 대신, mean latency + queue time + pending requests 조합으로 동일한 판단력을 확보했습니다.

- **알럿-대시보드를 한 세트로 운영하는 이유**: 알럿만 있으면 "뭔가 이상하다"까지만 알 수 있고, 대시보드만 있으면 "아무도 안 봅니다". Detect(알럿) -> Diagnose(Grafana) -> Act(대응)를 30초 루틴으로 고정해야 운영이 실제로 돌아갑니다.

---

## 면접 포인트

**Q. Alertmanager에서 dev/prod 교차 전송을 구조적으로 어떻게 차단하나요?**
A. Default receiver를 "null"로 두고, routes의 namespace regex matcher로 허용된 네임스페이스만 Slack으로 전송합니다. 매칭되지 않는 알럿은 자동으로 null로 떨어집니다.

**Q. Triton latency 알럿을 p95가 아닌 mean으로 잡은 이유는?**
A. Triton 기본 메트릭이 sum/count 형태라 histogram 기반 p95를 네이티브로 계산할 수 없습니다. mean latency에 queue time과 pending requests를 조합하면 병목 판단이 충분히 가능합니다.

**Q. Triton의 nv_inference_request_failure가 HTTP 4xx/5xx를 잡지 못하는 이유는?**
A. failure 카운터는 모델 실행 단계에 진입한 요청의 실패만 카운트합니다. 모델 미존재, shape mismatch 등은 실행 이전 단계에서 거부되므로 failure에 반영되지 않습니다. HTTP 실패율은 Gateway 계층 메트릭으로 별도 알럿을 구성해야 합니다.
