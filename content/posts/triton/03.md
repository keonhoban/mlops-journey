...
### Dev/Prod 분리 + Triton Serving Alerts

*(kube-prometheus-stack · PrometheusRule · Alertmanager · ArgoCD · SealedSecret)*

---

## 🧠 시나리오 설명

> 관측(Observability)은 “대시보드”가 아니라 **사고를 막는 운영 정책**입니다.
> 
> 
> 특히 dev/prod 환경에서 알럿이 섞이는 순간, 운영자는 신뢰를 잃고 결국 알럿을 무시하게 됩니다.
> 
> 이번 문서는 다음을 **영구적으로 보장**하는 운영 표준입니다.
> 
> - dev/prod 알럿 완전 분리
> - 라벨/네임스페이스 실수로 교차 전송이 발생해도 **Slack 전송 차단**
> - GitOps + SealedSecret 기반으로 **재현 가능한 표준화**
> - Triton 서빙 품질(성능/실패)을 **모델 실행 관점**에서 감지
> 
> 또한 알럿은 “발생”만으로 끝나지 않습니다.
> 운영에서는 알럿 이후 30초 안에 “원인 확인 → 조치 판단”이 가능해야 합니다.
> 
> 그래서 본 문서는 Alertmanager 라우팅 표준과 함께,
> Grafana 대시보드를 ‘알람 이후 판단 표준’으로 포함해
> Triton 서빙 품질을 지표 기반으로 확인 및 판단할 수 있도록 구성했습니다.
> 
...
histogram 기반 p95를 전제로 하기보다는 mean latency 기반으로 진행 했습니다.

(공식 문서: [https://docs.nvidia.com/deeplearning/triton-inference-server/user-guide/docs/user_guide/metrics.html](https://docs.nvidia.com/deeplearning/triton-inference-server/user-guide/docs/user_guide/metrics.html?utm_source=chatgpt.com))
...
- 만약, HTTP 실패율이 필요하면 Triton이 아니라 **Gateway 계층(Nginx/Envoy/Ingress)** 메트릭으로 별도 알럿을 둬야 합니다.
...
대신 **avg latency + queue time + pending requests** 같은 핵심 지표로 원인 확인을 가능하게 했습니다.
...
> 특히 model / version 변수는
> 
> 
> **MLflow → Triton 버전 디렉터리(version 라벨)** 흐름과 연결되어
> 
...
- “서비스 알럿은 namespace 라벨 필수”를 팀 룰로 고정하면 dev/prod 사고가 거의 사라집니다.
- Triton failure는 “모델 실행 실패”만 잡습니다. HTTP 실패율은 gateway 알럿으로 분리해야 합니다.
- Triton latency는 p95 대신 **avg latency + queue time + pending** 조합으로 판단한다.

---

## 🏁 정리

> PrometheusRule은 정책을 정의하고,
Alertmanager는 전파를 제어하며,
Grafana는 판단을 표준화합니다.
> 
> 
> Triton은 avg-latency로 감지하고,
> **queue/pending으로 병목 여부를 1차 분기**하며,
> error-rate는 운영 감시로만 유지합니다.
>
