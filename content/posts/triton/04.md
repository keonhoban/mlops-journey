...
preferred_batch_size [8, 16]으로 배치 오버헤드 감소와 대기 latency 증가 사이의 균형을 맞추고, instance_group count 2로 CPU 경합 없는 안정적 병렬 추론 시작값을 설정했다. Airflow Variable로 배치/인스턴스 설정을 런타임 제어하여 config.pbtxt 재생성 없이 A/B 성능 비교와 장애 대응 유연성을 확보했다.
