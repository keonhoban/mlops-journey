...
- **등록된 모델 ≠ 운영 모델이라는 원칙**: MLflow에 등록된 모델이 바로 운영 모델이 되면 서빙 상태를 고려하지 않은 배포가 된다. Triton READY 상태 + FastAPI reload 완료를 운영 모델의 조건으로 정의하여 서빙 안정성을 확보한다.
- **Triton explicit 모드 채택**: 자동 폴링(poll) 대신 명시적 load/unload를 사용한다. 파이프라인이 모델 로딩 시점을 정확히 제어할 수 있어 예기치 않은 모델 전환을 방지한다.
- **FastAPI와 Triton의 확장 전략 분리**: FastAPI는 stateless이므로 replica 수평 확장 + RollingUpdate, Triton은 NFS single-writer 제약으로 1 replica + Recreate 전략을 사용한다. 워크로드 특성에 맞는 확장 전략을 적용했다.

---

## 면접 포인트

- **Q: MLflow 등록과 Triton 배포를 분리한 이유는?**
  → MLflow는 모델 기록 시스템이고, Triton은 실제 서빙 런타임이다. 등록 시점과 배포 시점을 분리하면 READY Sensor로 모델 상태를 검증한 후에만 운영에 반영할 수 있다.

- **Q: Triton에서 explicit 모드를 사용하는 이유는?**
  → 기본 poll 모드는 model repository 변경을 자동 감지하여 모델을 로드한다. 이 경우 파이프라인 외부에서 예기치 않은 모델 전환이 발생할 수 있다. explicit 모드는 API 호출로만 모델을 로드하여 DAG이 전환 시점을 완전히 제어한다.

- **Q: FastAPI Reload API의 역할은?**
  → Triton served version을 확인하고, MLflow metadata를 조회하여 서빙 상태를 업데이트한다. FastAPI는 단순 inference gateway가 아니라 서빙 제어 계층이다.
