...
`/health`는 liveness 전용으로 프로세스 생존만 확인합니다.

모델 상태 상세 정보는 `/models` API에서 SSOT 기반으로 제공됩니다.
...
Triton은 NFS 공유 볼륨을 마운트하지만,

`--model-control-mode=explicit` 설정 덕분에

**NFS 디렉토리를 자동 폴링하지 않습니다.**

따라서 Pod 교체 중에도 부분 기록된 모델이 로드될 위험이 없어

RollingUpdate가 가능합니다.
...
`maxUnavailable: 0`으로 기존 Pod가 정상 상태에서만 새 Pod로 전환됩니다.

```
explicit 모드 → NFS 자동 폴링 없음 → RollingUpdate 안전
maxUnavailable: 0 → 무중단 교체
```
...
| Triton | RollingUpdate + 1 replica | explicit 모드 (NFS 자동 폴링 없음), maxUnavailable: 0 |
...
FastAPI `/predict` 핸들러는 Feast Feature Server와 HTTP로 연동됩니다.

`feast` 라이브러리를 직접 임포트하지 않고 REST API를 호출하여

Core 서비스가 Optional 레이어에 대한 라이브러리 의존 없이 동작합니다.

관련 코드:

```
charts/fastapi/app/services/feast_client.py
```

```python
# services/feast_client.py (HTTP 기반 — feast 라이브러리 의존 없음)
import httpx

async def get_online_features(entity_ids: list[str]) -> list[list[float]] | None:
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=settings.feast_timeout_sec) as client:
            resp = await client.post(
                f"{settings.feast_url}/get-online-features",
                json=payload,
            )
...
Feast Feature Server(Redis backend)에서 수행되므로 **ms 단위 latency**로 동작합니다.

실패 시 `None`을 반환하여 caller가 request payload를 그대로 사용합니다.
...
MLflow 등록 모델이 곧 운영 모델이 되지 않도록 Triton READY + FastAPI reload 완료를 운영 모델의 조건으로 정의하고, Triton explicit 모드로 파이프라인이 모델 로딩 시점을 정확히 제어한다. FastAPI와 Triton 모두 RollingUpdate(maxUnavailable: 0)로 무중단 배포하되, Triton은 explicit 모드 덕분에 NFS 자동 폴링 없이 안전하게 롤링 교체된다. Feature Store 연동은 HTTP 기반(Method C)으로 feast 라이브러리 의존 없이 Optional 레이어 탈부착 철학을 유지한다.
