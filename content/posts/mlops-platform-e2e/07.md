...
FastAPI는 stateless gateway이므로 수평 확장이 가능합니다.
...
CPU 사용률 70% 초과 시 replica가 2 → 5까지 자동으로 늘어납니다.

```
FastAPI replica 2 (평상시)
      ↓ CPU 70% 초과
FastAPI replica 3 → 4 → 5 (부하 증가 시)
```
...
| 컴포넌트 | 전략 | 이유 |
| --- | --- | --- |
| FastAPI | HPA (2~5 replica) | stateless, 수평 확장 가능 |
...
FastAPI `/predict` 핸들러에서 다음과 같이 Feast를 조회합니다.
...
| Optional OFF (Feature Store 없음) | 요청 payload의 feature를 직접 사용 → Triton 추론 |
