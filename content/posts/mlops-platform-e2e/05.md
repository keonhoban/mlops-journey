...
- [GitOps 기반 E2E ML Platform - 토글 구조](/mlops-journey/posts/mlops-platform-e2e/04/)
...
task가 아닌 **DAG 레벨**에서 설정되며, 실패 시 즉시 Slack 알림을 전송합니다.
...
- **학습 성공 ≠ 운영 반영이라는 전제**: train → deploy 직결 구조는 검증 없이 운영에 반영되는 위험이 있다. Drift Gate, Promotion/Shadow 분기, READY Sensor, Auto Rollback을 파이프라인 안에 내장해서 운영 리스크를 구조적으로 차단한다.
- **ids.py + policy.py SSOT 패턴**: task_id와 XCom key를 문자열로 하드코딩하면 TaskGroup prefix 변경 시 xcom_pull이 None을 반환하는 버그가 발생한다. ids.py에 식별자를, policy.py에 임계값을 집중시켜 변경 지점을 하나로 제한한다.
- **DAG 레벨과 Prometheus 레벨 2계층 에러 감지**: on_failure_callback/SLA는 파이프라인 실행 이상을, Prometheus Auto Rollback은 서빙 품질 이상을 감지한다. 서로 다른 계층에서 에러를 감지하여 빈틈을 줄인다.

---

## 면접 포인트

- **Q: 이 파이프라인이 일반적인 ML 파이프라인과 다른 점은?**
  → 학습 파이프라인이 아니라 운영 반영 제어 파이프라인이다. Train과 Deploy 사이에 Drift Gate, Promotion Check, READY Sensor, Observability, Rollback이 있어 검증 없이 배포되지 않는다.

- **Q: ids.py와 policy.py를 분리한 이유는?**
  → ids.py는 코드 구조 변경 시(task_id, XCom key), policy.py는 운영 정책 변경 시(임계값, 파라미터) 수정된다. 변경 이유가 다르므로 파일을 분리한다.

- **Q: Auto Rollback과 Manual Rollback의 차이는?**
  → Auto Rollback은 E2E DAG 내부 task로 trigger_rule=ONE_FAILED에 의해 자동 실행된다. Manual Rollback은 별도 DAG(rollback_manual.py)으로 운영자가 직접 실행한다.

---

## 다음에 읽을 글

→ [GitOps 기반 E2E ML Platform - 모델 반영 제어](/mlops-journey/posts/mlops-platform-e2e/06/) — Drift Gate / Promotion / Shadow 상세 설계
