...
train에서 deploy로 직결하는 구조는 검증 없이 운영에 반영되는 위험이 있으므로, Drift Gate/Promotion-Shadow 분기/Auto Rollback을 파이프라인에 내장하여 운영 리스크를 구조적으로 차단했다. Core/Baseline/Optional 레이어 분리로 확장 기능 장애가 코어 서빙에 전파되지 않도록 격리했다.
