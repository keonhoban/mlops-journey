...
- **설계 ≠ 실행이라는 관점**: 아키텍처 다이어그램과 코드 설명만으로는 시스템이 실제로 동작하는지 증명할 수 없다. Serving Runtime 상태를 Proof 파일(docs/proof/latest/e2e_success/)로 기록하여 재현 가능한 검증 기록을 남긴다.
- **dev/prod 양 환경을 모두 검증**: 단일 환경만 확인하면 환경 차이로 인한 문제를 놓칠 수 있다. dev와 prod 양쪽의 Triton/FastAPI 상태를 동시에 확인하여 환경 일관성을 검증한다.
- **부하 테스트까지 포함한 검증 범위**: 단일 요청 성공은 운영 안정성을 보장하지 않는다. k6 부하 테스트로 136 RPS, p95 553ms, 에러율 0%를 측정하여 실제 트래픽 환경에서의 동작을 확인했다.
...
- **Q: Proof 디렉토리의 목적은?**
  → 플랫폼의 동작을 문서가 아닌 실행 결과 파일로 증명한다. Triton READY 상태, FastAPI health, Reload API 응답 등을 기록하여 "이 시스템이 실제로 동작했다"는 증거를 남긴다.

- **Q: Serving Runtime이 정상인지 어떻게 확인하는가?**
  → Triton은 /v2/health/ready(서버 상태)와 /v2/repository/index(모델 목록), FastAPI는 /health(서버 상태), /models(서빙 상태), /metrics(Prometheus 지표)를 확인한다.

- **Q: 부하 테스트 결과에서 p95 553ms의 원인은?**
  → CPU-only 3노드 환경에서 100 VU 부하 시 CPU 추론 큐가 쌓이면서 발생한다. GPU 환경에서는 병렬 처리와 dynamic_batching 효율 극대화로 50ms 이하로 개선이 예상된다.
