...
> **참고:** dev 환경은 단일 Triton 인스턴스에 `best_model`과 `best_model_shadow` 두 모델명을 배포합니다.
> 반면 prod 환경은 별도 Triton 인스턴스(`triton-prod`/`triton-dev`)를 운영하며,
> 두 인스턴스 모두 동일한 모델명(`best_model`)을 사용합니다.
