...
```
best_model version 46 — READY 상태
```

즉 Triton Model Repository에서

모델이 명시적으로 로드되어 서빙 가능 상태입니다.

> **참고:** prod/shadow 분기는 별도 모델명이 아니라
> prod Triton 인스턴스와 shadow Triton 인스턴스를 분리하여 구현합니다.
> 두 인스턴스 모두 동일한 모델명(`best_model`)을 사용하며,
> FastAPI의 `traffic_mode`(mirror/split)로 트래픽을 제어합니다.
...
```
{
 ...
 "effective":{
   "A":{ "mode":"ssot","version":46,"run_id":"e574739..." },
   "B":{ "mode":"ssot","version":46,"run_id":"e574739..." }
 }
}
```
