...
> HPA 템플릿은 구현 완료 상태이며, 의도적으로 `enabled: false`로 비활성화되어 있습니다.
> 2 replica로 136 RPS / 에러율 0%를 처리하는 현 환경에서는
> 스케일 아웃보다 노드 리소스 경합 방지가 우선이기 때문입니다.
> 활성화 시점의 검증을 위해 HPA 전용 부하 테스트(`ops/load-test/k6-scaling.js`, 200 VU)도 준비되어 있습니다.
