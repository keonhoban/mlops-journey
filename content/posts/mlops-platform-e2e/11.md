...
- [GitOps 기반 E2E ML Platform - 운영 문서화](/mlops-journey/posts/mlops-platform-e2e/10/)
...
```yaml
# charts/fastapi/templates/hpa.yaml (향후 적용 예정 — 현재 미생성)
spec:
  minReplicas: 2
  maxReplicas: 5
  metrics:
...
> **참고:** 이 HPA 템플릿은 아직 실제 Helm 차트에 생성되지 않은 상태입니다.
> 부하 테스트 결과를 바탕으로 향후 적용할 설계 기준을 정리한 것입니다.
...
- **부하 테스트를 E2E 검증에 포함시킨 이유**: 단일 요청 성공과 100명 동시 요청은 전혀 다른 문제다. API 호출 성공만으로는 운영 트래픽에서의 안정성을 보장할 수 없으므로 부하 테스트를 Proof의 일부로 포함했다.
- **Ramp-up → Sustained → Ramp-down 3단계 구성**: 점진적 부하 증가로 시스템 워밍업을 하고, 최대 부하에서 처리량과 에러율을 측정하며, 부하 해소 후 시스템 복구를 확인한다. 실제 트래픽 패턴에 가까운 테스트 구조다.
- **HPA 설계 기준과 연결**: 부하 테스트 결과를 바탕으로 HPA 트리거 기준(CPU 70%, replica 2~5)을 설정했다. 실측 데이터 없이 임의로 설정하면 과잉/부족 확장이 발생한다.

---

## 면접 포인트

- **Q: CPU-only 환경에서 p95 553ms의 원인은?**
  → Triton 추론 엔진은 GPU에 최적화되어 있다. CPU 환경에서는 thread 기반 병렬 처리로 코어 수 제한에 따른 배치 처리 병목이 발생한다. Peak 부하 구간에서 CPU 추론 큐가 쌓이면서 p95가 증가한다.

- **Q: 에러율 0%가 의미하는 것은?**
  → 20,553건 전량 성공이므로 서빙 시스템의 안정성이 확인된다. 다만 이는 CPU-only 환경의 처리 한계(136 RPS) 내에서의 결과이며, 이 이상의 트래픽에서는 HPA를 통한 수평 확장이 필요하다.

- **Q: GPU 환경으로 전환하면 어떤 개선이 예상되는가?**
  → GPU 병렬 처리와 dynamic_batching 효율 극대화로 p95 50ms 이하, 처리량 수백 RPS 이상이 기대된다.
