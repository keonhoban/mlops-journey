...
```
Ramp-up:   0 → 50 VU   (30초)
Sustained: 50 VU 유지   (60초)
Peak:      50 → 100 VU  (30초) → 100 VU 유지 (60초) → 0 (30초)
```
...
Sustained: 안정 상태 처리량 측정
Peak:      최대 부하 상태에서 에러율 확인
```
...
docs/proof/load-test/k6_result_detail.json
```
...
```
docs/proof/load-test/
 ├─ k6_result_summary.txt
 └─ k6_result_detail.json
```

`k6_result_summary.txt` 주요 내용:
