...
단일 요청 성공은 운영 안정성을 보장하지 않으므로 Ramp-up/Sustained/Ramp-down 3단계 부하 테스트를 Proof의 일부로 포함했다. 부하 테스트 실측 데이터를 기반으로 HPA 트리거 기준(CPU 70%, replica 2~5)을 설정하여 임의 설정으로 인한 과잉/부족 확장을 방지했다.
