...
- **Shadow 경로로 운영 리스크 분산**: 성능 기준 미달이나 drift 감지 시 즉시 차단하지 않고 shadow 경로로 이동시킨다. shadow에서 실제 트래픽으로 모델을 검증하되 사용자 응답에는 영향을 주지 않아 안전하게 실험할 수 있다.
...
- **Q: Promotion과 Shadow의 분기 기준은?**
  → drift 없음 AND accuracy >= threshold이면 Promotion(운영 경로), drift 감지 또는 accuracy 미달이면 Shadow(테스트 경로), 심각한 오류(학습 실패, artifact 문제)면 즉시 중단이다.
