...
데이터 분포 변화가 모델 성능에 직접 영향을 주므로 Drift Gate를 모델 성능 검증보다 먼저 배치하고, 기준 미달 시 Shadow 경로로 운영 리스크를 분산한다. client_id 기반 SHA256 deterministic routing으로 동일 사용자가 항상 같은 모델로 라우팅되어 A/B 테스트 결과의 일관성을 보장한다.
