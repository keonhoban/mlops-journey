...
- [GitOps 기반 E2E ML Platform - 운영 환경 반영 제어](/mlops-journey/posts/mlops-platform-e2e/05/)
...
**p-value를 함께 계산**하여 통계적 유의성을 보강합니다.
...
- D 값만으로는 알 수 없는 **샘플 크기 대비 통계적 신뢰도**를 제공
...
→ canary 경로 (소수 트래픽으로 실검증 후 전체 전환)

canary 관련 정책은 다음 Variable로 관리됩니다.

```
canary_traffic_pct
canary_observe_window_sec
canary_promote_threshold
```
...
- **Drift Gate를 모델 성능 검증보다 먼저 배치**: accuracy가 높아도 데이터 분포가 바뀌었으면 운영 환경에서 성능이 급격히 떨어질 수 있다. 데이터 품질을 먼저 확인하고 모델 성능을 평가하는 순서가 더 안전하다.
- **Canary/Shadow 경로로 운영 리스크 분산**: 성능 기준 미달이나 drift 감지 시 즉시 차단하지 않고 canary 또는 shadow 경로로 이동시킨다. canary는 소수 트래픽으로 실검증하고, shadow는 사용자 응답에 영향 없이 안전하게 실험할 수 있다.
- **client_id 기반 deterministic routing**: 랜덤 라우팅 대신 client_id의 SHA256 해시를 사용해 동일 사용자가 항상 같은 모델로 라우팅된다. A/B 테스트 결과의 일관성을 보장한다.

---

## 면접 포인트

- **Q: Drift Gate에서 KS statistic을 사용한 이유는?**
  → KS statistic은 두 분포 사이의 최대 거리를 측정하는 비모수 검정이다. 분포 가정 없이 데이터 분포 변화를 감지할 수 있어 다양한 feature에 범용적으로 적용 가능하다.

- **Q: Promotion/Canary/Shadow의 분기 기준은?**
  → drift 없음 AND accuracy >= promote_threshold이면 Promotion, threshold ≤ accuracy < promote_threshold이면 Canary(소수 트래픽 실검증), accuracy 미달 또는 drift 감지면 Shadow, 심각한 오류면 즉시 중단이다.

- **Q: p-value를 추가한 이유는?**
  → KS D 값은 분포 차이의 크기만 나타내고, 그 차이가 통계적으로 유의한지는 알려주지 않는다. p-value를 함께 보면 "이 차이가 우연일 확률"까지 판단할 수 있다. asymptotic 근사로 scipy 의존 없이 구현했다.

- **Q: Canary 경로는 언제 사용하나?**
  → Promotion 최소 조건은 충족하지만 최상위 기준에는 미달할 때 사용한다. 소수 트래픽을 새 모델로 라우팅하여 실제 환경에서 성능을 확인한 후 전체 전환한다.

- **Q: Mirror와 Split 전략의 차이는?**
  → Mirror는 모든 요청을 production 모델이 처리하고 shadow 모델은 background에서 추론만 한다. Split은 일부 트래픽을 실제로 shadow 모델이 처리한다. Mirror는 사용자 영향 제로, Split은 A/B 테스트에 가깝다.

---

## 다음에 읽을 글

→ [GitOps 기반 E2E ML Platform - 모델 등록/서빙 반영 분리](/mlops-journey/posts/mlops-platform-e2e/07/) — MLflow + Triton + FastAPI 서빙 구조
