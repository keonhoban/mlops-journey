...
```
platform repo — docs/runbook/
 ├─ observability.md
 └─ optional-toggle.md

dags repo — docs/
 ├─ 02_deployment_flow.md
 ├─ 04_rollback_policy.md
 └─ 06_runbook.md
```
...
README만으로는 장애 대응과 보안 정책을 전달할 수 없으므로 Runbook/Security/Proof를 디렉토리 구조로 분리하여 문서 종류별 위치를 강제했다. Proof는 make proof-e2e 스크립트로 자동 생성하여 수동 기록의 누락 없이 재현 가능한 검증 기록을 남긴다.
