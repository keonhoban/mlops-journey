...
- [GitOps 기반 E2E ML Platform - 설계 의도](/mlops-journey/posts/mlops-platform-e2e/01/)
...
- **기능이 아닌 운영 책임 기준으로 레이어 분리**: 도구 종류가 아니라 "장애 시 영향 범위"와 "변경 빈도"를 기준으로 Core/Baseline/Optional을 나눴다. Feature Store 장애가 서빙을 멈추지 않도록 구조적으로 격리한다.
- **레이어 구조를 ArgoCD AppProject에 직접 반영**: 문서상 레이어가 아니라 GitOps 배포 구조 자체에 경계를 반영했다. 프로젝트 수준에서 namespace와 리소스 접근을 제한하여 잘못된 배포를 구조적으로 방지한다.
- **Baseline을 Core와 분리한 이유**: Monitoring, Logging, Storage는 ML 기능과 무관하지만 운영에 필수다. Core에 포함하면 ML 파이프라인 변경 시 관측 시스템까지 영향받을 위험이 있다.

---

## 면접 포인트

- **Q: Core / Baseline / Optional을 나눈 기준은?**
  → 운영 책임과 장애 전파 범위. Core는 모델 생명주기, Baseline은 항상 가동되어야 하는 운영 기반, Optional은 제거해도 서빙에 영향 없는 확장 기능이다.

- **Q: Optional 레이어를 제거하면 어떤 일이 일어나는가?**
  → Core(Airflow, MLflow, Triton, FastAPI)와 Baseline(Monitoring, Logging, Storage)은 정상 동작한다. 이를 실제 Proof(docs/proof/latest/core_only/)로 검증했다.

- **Q: 이 레이어 구조가 ArgoCD에서 어떻게 구현되어 있는가?**
  → AppProject(dev, prod, baseline, optional)로 배포 경계를 정의하고, 각 프로젝트는 허용된 namespace에만 리소스를 배포할 수 있다.

---

## 다음에 읽을 글

→ [GitOps 기반 E2E ML Platform - GitOps 구조](/mlops-journey/posts/mlops-platform-e2e/03/) — ArgoCD AppProject / ApplicationSet으로 배포 경계 강제
