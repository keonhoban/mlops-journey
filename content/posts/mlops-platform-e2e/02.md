...
도구 종류가 아닌 장애 영향 범위와 변경 빈도를 기준으로 Core/Baseline/Optional을 분리하고, 이 레이어 경계를 ArgoCD AppProject에 직접 반영하여 잘못된 배포를 구조적으로 방지한다. Baseline(Monitoring, Logging, Storage)은 ML 기능과 무관하지만 운영에 필수이므로 Core와 분리하여 상호 영향을 차단했다.
