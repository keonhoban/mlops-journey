...
- **비파괴 detach 방식 채택**: Optional OFF 시 namespace를 삭제하지 않고 Application만 제거한다. namespace를 삭제하면 PVC, Storage 데이터, Access 정책이 함께 사라져 복구가 어렵다.
- **ArgoCD Application 삭제로 Detach 구현**: Helm uninstall이 아닌 ArgoCD Application 삭제 방식을 사용한다. GitOps 상태와 클러스터 상태를 일치시키면서도 Optional 리소스만 깔끔하게 제거할 수 있다.
- **Core 독립성을 Proof로 검증**: Optional OFF 후 FastAPI health, Triton readiness를 실제로 확인하여 Core가 Optional에 의존하지 않음을 실행 결과로 증명했다.

---

## 면접 포인트

- **Q: Optional OFF 시 namespace를 왜 유지하는가?**
  → namespace 삭제는 PVC와 데이터까지 제거한다. 비파괴 detach로 Application만 삭제하면 데이터를 보존하면서 기능만 분리할 수 있다.

- **Q: Feature Store를 제거해도 서빙이 되는 이유는?**
  → FastAPI는 Feast 연결 실패 시 요청 payload의 feature를 직접 사용하는 fallback 구조를 가진다. Optional 레이어가 Core 서빙 경로의 필수 의존성이 아니다.

- **Q: Optional ON/OFF를 어떻게 검증했는가?**
  → 실제 ArgoCD 환경에서 Optional 앱 삭제 후 Core health probe(FastAPI /health, Triton /v2/health/ready)를 실행하여 정상 동작을 확인했다. 결과는 docs/proof/latest/core_only/에 기록했다.
